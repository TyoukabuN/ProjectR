//-----------------------------------------------------------------------
// <copyright file="InspectorConfig.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public class InspectorConfig : GlobalConfig<InspectorConfig>, ISerializationCallbackReceiver
    {
        ...
        {
            if (IsHeadlessMode || UnityEditorInternal.InternalEditorUtility.inBatchMode)
            {
                return;
            }
            ...
            Type iPropertyViewInterface = typeof(EditorWindow).Assembly.GetType("UnityEditor.IPropertyView");
            ...
            if (iPropertyViewInterface != null && activeEditorTrackerType != null)
            {
                // Since UnityEditor.IPropertyView is introduced in this version, we simply update all editor windows implementing that interface, as
                // there are now more kinds of windows than UnityEditor.InspectorWindow which need to be updated, for example UnityEditor.PropertyEditor.

                var forceRebuild = activeEditorTrackerType.GetMethod("ForceRebuild", Flags.InstanceAnyVisibility);
                var trackerProp = iPropertyViewInterface.GetProperty("tracker", Flags.InstanceAnyVisibility);

                if (forceRebuild != null && trackerProp != null)
                {
                    didUpdate = true;
                    ...
                    foreach (var window in windows)
                    {
                        try
                        {
                            if (iPropertyViewInterface.IsAssignableFrom(window.GetType()))
                            {
                                object tracker = trackerProp.GetValue(window);
            ...
            if (!didUpdate && inspectorWindowType != null && activeEditorTrackerType != null)
            {

...
3. 输出文件的存放目录（默认 `.document/research/<游戏缩写>/`）
...
所有文件写完后，为每个子文档的**开头和结尾**加上导航链接。

**导航格式**：
```markdown
# 章节标题

[<< 上一章](上一文件.md) · [返回索引](00_index.md) · [下一章 >>](下一文件.md)
```

**三种情况**：
- 第一章（`01_`）：开头导航去掉"<< 上一章"，改为只留 `[返回索引] · [下一章 >>]`
- 中间章：三项齐全 `[<< 上一章] · [返回索引] · [下一章 >>]`
- 最后一章（末尾文件）：去掉"下一章 >>"，只留 `[<< 上一章] · [返回索引]`

**注意**：
- 导航放在章节标题正下方（不是文件最顶部）
- 末尾导航放在 `---` 分割线之后
- 拆分大文件时，末尾可能残留下一文件的章节标题，加导航时同步删除这些残留标题
...
| 文件末尾有残留的下一章标题 | 大文件按行号拆分时边界多切了一行，加导航时顺手删除 |
| search_replace 找不到唯一匹配 | 扩大 original_text 范围，包含更多上下文 |
| 需要批量替换导航符号 | **不要用 PowerShell 批量替换**，`→` 等 Unicode 符号会匹配到汉字偏旁（如"引"），炸掉正文。正确做法：逐文件用 search_replace，每次只替换导航行本身（original_text 包含完整链接文字确保唯一） |
| 避免符号替换炸文 | 导航符号从一开始就用纯 ASCII 写（`<<` / `返回索引` / `>>`），彻底杜绝此问题 |

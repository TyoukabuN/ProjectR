...
#pragma warning disable
    ...
    [System.Diagnostics.Conditional("UNITY_EDITOR")]
    public class DynaEnumTextAttribute : Attribute
    {
        public interface IResolver
        {
            string Resolve(string key);
        }
        public static System.Collections.Generic.Dictionary<Type, IResolver> rs = new System.Collections.Generic.Dictionary<Type, IResolver>();

        
        public readonly Type resolverType;
        public string Key;
        public DynaEnumTextAttribute(Type resolverType)
        {
            this.resolverType = resolverType ?? throw new ArgumentNullException(nameof(resolverType));
            if (!rs.ContainsKey(resolverType))
            {
                rs.Add(resolverType,Activator.CreateInstance(resolverType) as IResolver);
            }
        }
    }

//-----------------------------------------------------------------------
// <copyright file="ASTEmitter.cs" company="Sirenix ApS">
...
#pragma warning disable
...
#if SIRENIX_INTERNAL
                var returnTypeOverrideAttrs = method.GetCustomAttributes(typeof(Internal.InternalCommandStateIndexerReturnTypeOverrideAttribute), false);

                if (returnTypeOverrideAttrs != null && returnTypeOverrideAttrs.Length > 0)
                {
                    if (elementArgs.Length != 1 || ast.Children[1].NodeType != NodeType.CONSTANT_STRING) throw new SyntaxException(ast, "An access to an indexer with a return type eval override can only have one argument which is a string constant");

                    string stringArg = (string)ast.Children[1].NodeValue;
                    var attr = returnTypeOverrideAttrs[0] as Internal.InternalCommandStateIndexerReturnTypeOverrideAttribute;

                    MethodInfo evalOverrideMethod = container.GetMethod(attr.EvalMethod, Flags.StaticAnyVisibility, null, new Type[] { typeof(string) }, null);

                    if (evalOverrideMethod == null) throw new SyntaxException(ast, "Couldn't find eval override type method '" + attr.EvalMethod + "' in type '" + container.GetType().GetNiceFullName() + "'");
                    if (evalOverrideMethod.ReturnType != typeof(Type)) throw new SyntaxException(ast, "Eval override type method '" + attr.EvalMethod + "' in type '" + container.GetType().GetNiceFullName() + "' must return System.Type");

                    Type returnTypeOverride = (Type)evalOverrideMethod.Invoke(null, new object[] { stringArg });

                    if (returnTypeOverride == null) throw new SyntaxException(ast, "Override return type is null");
                    if (!method.ReturnType.IsAssignableFrom(returnTypeOverride)) throw new SyntaxException(ast, "Override return type '" + returnTypeOverride.GetNiceFullName() + "' is not assignable to its base return type '" + method.ReturnType.GetNiceFullName() + "'");

                    ast.TypeOfValue = returnTypeOverride;
                    ...
                    var castAst = new ASTNode()
                    {
                        NodeType = NodeType.TYPE_CAST,
                        NodeStartIndex = ast.NodeStartIndex,
                        NodeEndIndex = ast.NodeEndIndex,
                    };

                    castAst.Children[0] = new ASTNode()
                    {
                        NodeValue = returnTypeOverride,
                        NodeStartIndex = ast.NodeStartIndex,
                        NodeEndIndex = ast.NodeEndIndex,
                    };

                    castAst.Children[1] = new ASTNode()
                    {
                        TypeOfValue = method.ReturnType,
                        NodeStartIndex = ast.NodeStartIndex,
                        NodeEndIndex = ast.NodeEndIndex,
                    };

                    TypeCast(castAst, true);
                }
                else
#endif
                {
        ...
        private static MemberInfo ResolveIdentifier(ASTNode ast, MemberInfo context, string identifier, bool onlyLocals, Type[] genericArgs)
        {
            ...
            List<Type> typesWithShortName;
            {
                ...
                if (IdentifierLookups.TypesByShortName.TryGetValue(useIdentifier, out typesWithShortName))
                {
                    ...
                    else if (useIdentifier == "Debug")
                    {
                        return typeof(UnityEngine.Debug);
                    }
                    else if (useIdentifier == "Color")
                    {
    ...
    #region Operator Utilities
        ...
        {
            this.actions.Add(il => il.Emit(OpCodes.Nop));

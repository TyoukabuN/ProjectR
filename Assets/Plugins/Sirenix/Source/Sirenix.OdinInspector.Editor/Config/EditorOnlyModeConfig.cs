//-----------------------------------------------------------------------
// <copyright file="EditorOnlyModeConfig.cs" company="Sirenix ApS">
...
#pragma warning restore CS0414
...
#if EDITOR_ONLY_BUILD
        [InfoBox(
            "This build is an editor only distribution of Odin, meaning that Editor Only Mode cannot be toggled on or off.\n\n" +
            
            "Editor Only Mode is a version of Odin that contains only the parts that Odin needs to function in the editor, and has " +
            "all runtime libraries stripped out, except for the very small Sirenix.OdinInspector.Attributes assembly. This means the " +
            "serializer and utility libraries are absent at runtime and do not affect your build. Note that they are still present in " +
            "the editor, though, as Odin's inspector systems use them internally.\n\n" +

            "You can download a non-Editor Only package from https://odininspector.com/downloads")]
#else
        [InfoBox(
            ...
            "editor functionality that Odin provides. This will also let you use Odin while targeting non-IL2CPP UWP platform targets.\n\n" +
        ...
        public void Update()
        {
            ...
            // Finds all platform specific assemblies which are located in special folders, These files are never active in the Editor.
            {
                ...
                this.platformSpecificAssemblyFiles = new string[]
                {
                ...
                {
                    var item = this.platformSpecificAssemblyFiles[i];
                    if (File.Exists(item))
                    {
                        this.platformSpecificAssemblyFiles[n] = item;
                        n++;
                    }
                }
                Array.Resize(ref this.platformSpecificAssemblyFiles, n);
            }
        ...
        {
#if EDITOR_ONLY_BUILD
            throw new InvalidOperationException("Cannot disable editor only mode in an editor only build.");
#else
            ...
            UnityEditorEventUtility.EditorApplication_delayCall += () =>
            {
                ...
                // Move script files out of editor folder and toggle assembly definition back on.
                {
                    ...
                    foreach (var file in SerializerScriptFiles)
                    {
                        var from = sourceFolder + file;
                        var to = targetFolder + file;
                        ...
                        //    {
                        //        if (File.Exists(to + ".meta")) File.Delete(to + ".meta");
                        //        File.Move(from + ".meta", to + ".meta");
                        //    }
                        //}
                    }

                    var asmdef = targetFolder + RuntimeAssemblyDefinitionFile;
                    var asmdefBackup = asmdef + BACKUP_FILE_SUFFIX;

                    MoveAsset(asmdefBackup, asmdef);
                }
                ...
                ImportSettingsConfig.Instance.ApplyImportSettings();
            };
#endif
        }
        ...
        public void EnableEditorOnlyMode(bool force)
        {
            ...
            // Move script files to Editor folder and toggle assembly definition file off.
            {
                ...
                var asmdef = sourceFolder + RuntimeAssemblyDefinitionFile;
                var asmdefBackup = asmdef + BACKUP_FILE_SUFFIX;

                MoveAsset(asmdef, asmdefBackup);
            }
        ...
        private bool ShowEnableEditorOnlyMode
        {
            get
            {
                return
#if !EDITOR_ONLY_BUILD
                    !this.isInEditorOnlyMode &&
#endif
                    !this.isUsingSourceCode;
            }
        }

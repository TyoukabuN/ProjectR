//-----------------------------------------------------------------------
// <copyright file="PropertyValueCollectionAlias.cs" company="Sirenix ApS">
    ...
    public sealed class PropertyValueCollectionAlias<TActualValue, TValue> : PropertyValueCollection, IPropertyValueCollection<TValue> //where TValue : TActualValue
    {
        ...
        public new TValue this[int index]
        {
            get
            {
                try
                {
                    if (this.aliased[index] is TValue val)
                        return val;
        ...
        public override void RevertUnappliedValues()
        {
            ...
            if (IsMarkedAtomic)
            {
                ...
                {
                    if (!(this.aliased[i] is TValue value)) throw new InvalidCastException();
        ...
        private class OriginalValuesAlias : IImmutableList<TValue>
        {
            ...
            TValue IList<TValue>.this[int index]
            {
                get 
                { 
                    if (this.aliased[index] is TValue val) return val;
                    throw new InvalidCastException();
                }
                set
                {
            ...
            bool ICollection<TValue>.Remove(TValue item)
            {
                if (!(item is TActualValue val)) throw new InvalidCastException();
                return this.aliased.Remove(val);
            }

...
// </copyright>
//-----------------------------------------------------------------------
namespace Hunter.OdinEditorMod
{
#pragma warning disable
    ...
    /// </summary>
    public class HSEnumToggleButtonsAttributeDrawer<T> : OdinAttributeDrawer<HunterEnumToggleButtonsAttribute, T>
    {
        ...
        private ValueResolver<string> _stringResolver;
        ...
        {
            this._stringResolver = ValueResolver.GetForString(this.Property, this.Attribute._extraLabelStr);
        ...
        private void UpdateExtra()
        {
            ...
            var enumNames = Enum.GetNames(enumType);
            //添加自定义string[]
            var tempNames = new List<string>(enumNames.ToList()).ToArray();
            var tempTooltips = new List<string>(enumNames.ToList()).ToArray();
            if (this.Attribute._customNames != null)
            {
                for (int i = 0; i < this.Attribute._customNames.Length; i++)
                {
                    if (i < tempNames.Length && !string.IsNullOrEmpty(this.Attribute._customNames[i]))
                        tempNames[i] = this.Attribute._customNames[i];
                }
            }
            else
            {
                string[] extraStrs = null;
                if (!string.IsNullOrEmpty(this._stringResolver.GetValue()))
                    extraStrs = this._stringResolver.GetValue().Split(',');

                //根据label
                FieldInfo[] fieldInfos = enumType.GetFields();
                var weaponTypes = enumType.GetEnumValues();
                
                for (int i = 0; i < fieldInfos.Length - 1; i++)
                {
                    if (i < tempNames.Length && fieldInfos[i + 1].GetAttribute<LabelTextAttribute>() != null)
                    {
                        tempNames[i] = fieldInfos[i + 1].GetAttribute<LabelTextAttribute>().Text;
                        tempTooltips[i] = tempNames[i];
                        if (fieldInfos[i + 1].GetAttribute<TooltipAttribute>() is TooltipAttribute tt)
                        {
                            tempTooltips[i] = tt.tooltip;
                        }
                        //额外显示
                        if (this.Attribute._useExtraHelper && extraStrs != null && i < extraStrs.Length && !string.IsNullOrEmpty(extraStrs[i]))
                        {
                            tempNames[i] = extraStrs[i].Replace("$", tempNames[i]);
                        }
                    }
                }
            }
            this.Names = tempNames.Select(x => new GUIContent(Sirenix.Utilities.StringExtensions.SplitPascalCase(x))).ToArray();
            for (int i = 0; i < this.Names.Length; i++)
            {
                this.Names[i].tooltip = tempTooltips[i];
            }
        }
        ...
        protected override void DrawPropertyLayout(GUIContent label)
        {
            ...
            bool specifyHeight = Attribute.specifyHeight > 0f;
            i = 0;
            for (int j = 0; j < this.ColumnCounts.Count; j++)
            {
                ...
                if (specifyHeight)
                {
                    SirenixEditorGUI.GetFeatureRichControlRect(j == 0 ? label : GUIContent.none, out id, out hasFocus, out rect, GUILayout.Height(Attribute.specifyHeight));
                }
                else
                {
    ...
    // public static class EnumToggleButtonsHelper
    // {
    //     public static Dictionary<string, string> mEnumExtraDict = new Dictionary<string, string>();
    // }
}
#endif
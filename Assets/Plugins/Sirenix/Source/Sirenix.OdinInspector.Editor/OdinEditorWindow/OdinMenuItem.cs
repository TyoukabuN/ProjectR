//-----------------------------------------------------------------------
// <copyright file="OdinMenuItem.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public class OdinMenuItem
    {
        ...
        #region Modified By Hunter (jb) -- 2022年3月24日

        public bool clampLabelWidth;
        public float clampLabelWidthValue;
        public bool showLabelToolTip;
        public string tooltip;
        private bool disbableInMenu;
        ...
        //禁用菜单中的选择
        public bool DisbableInMenu
        {
            get => disbableInMenu;
            set => disbableInMenu = value;
        }
        #endregion
        ...
        public virtual void DrawMenuItem(int indentLevel)
        {
            ...
            if (eType == EventType.Repaint)
            {
                ...
                {
                    EditorGUI.DrawRect(this.rect, SirenixGUIStyles.MouseOverBgOverlayColor);
                }
                ...
                if (ico)
                {
                    #region Modified By Hunter (jb) -- 2023年12月2日

                    this.iconRect = this.labelRect.AlignLeft(this.Style.IconSize).AlignMiddle(this.Style.IconSize);
                    ...
                    iconRect = iconRect.AlignCenter(Mathf.Min(iconRect.width, ico.width), Mathf.Min(iconRect.height, ico.height));
                ...
                #region Modified By Hunter (jb) -- 2023年2月15日

                if (clampLabelWidth)
                {
                    this.labelRect = this.labelRect.AlignLeft(clampLabelWidthValue);
                }
                ...
                if (showLabelToolTip)
                {
                    GUI.Label(this.labelRect, GUIHelper.TempContent(SmartName,tooltip??SmartName) , labelStyle);    
                }
                else
                {
        ...
        public void Remove()
        {
            if (this == this.MenuTree.Root)
            {
                throw new Exception("Cannot remove root menu item!");
            }

            this.MenuTree.Selection.Remove(this);
            var parent = this.Parent ?? this.MenuTree.RootMenuItem;
            parent.ChildMenuItems.Remove(this);
            this.MenuTree.MarkDirty();
        }

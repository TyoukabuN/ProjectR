...
namespace PJR
{
    ...
    public partial class ProjectHierarchyExtension : MonoBehaviour
    {
        ...
        {
            CheckAssetPath(guid, selectionRect);

            CheckAssetType(guid, selectionRect);
        }
        ...
        static void CheckAssetPath(string guid, Rect selectionRect)
        {
            string path = AssetDatabase.GUIDToAssetPath(guid);
            if (!AssetDatabase.IsValidFolder(path))
                return;
            if (folderPath2desc.TryGetValue(path, out var content))
            {
        ...
        /// <param name="guid"></param>
        /// <param name="selectionRect"></param>
        static void CheckAssetType(string guid, Rect selectionRect)
        {
            string path = AssetDatabase.GUIDToAssetPath(guid);

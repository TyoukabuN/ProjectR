...
#pragma warning disable
    ...
    public static class GUIHelper
    {
        ...
        static GUIHelper()
        {
            ...
            GUILayoutEntriesCursorIndexGetter = DeepReflection.CreateValueGetter<int>(typeof(GUILayoutUtility), "current.topLevel.m_Cursor", false);
            GUILayoutEntriesGetter = DeepReflection.CreateValueGetter<IList>(typeof(GUILayoutUtility), "current.topLevel.entries", false);
            GetTopLevelLayoutRectGetter = DeepReflection.CreateValueGetter<Rect>(typeof(GUILayoutUtility), "current.topLevel.rect", false);
        ...
        #region Modified By Hunter (jb) -- 2022年4月20日

        private class FakePingObject:ScriptableObject{}
        public static void HardPingFolder(string folderPath)
        {
            var systemSubDir = folderPath;
            if (systemSubDir.StartsWith("Assets/"))
            {
                systemSubDir = folderPath.Remove(0, "Assets/".Length);
            }

            if (!systemSubDir.StartsWith("/"))
            {
                systemSubDir = "/" + systemSubDir;
            }
            var fullPath = Application.dataPath + systemSubDir; 
            if (!System.IO.Directory.Exists(fullPath)) return;
                
            var fakePingerObject = ScriptableObject.CreateInstance<FakePingObject>();
            var path = folderPath + "/fake";
            try
            {
                AssetDatabase.CreateAsset(fakePingerObject, path);
            ...
            {
                EditorGUIUtility.PingObject(fakePingerObject);
        ...
        #region Modified By Hunter (jb) -- 2022年9月21日

        public static bool IsSelectedTab(EditorWindow window)
        {
            if (window == null) return false;
            var method = typeof(EditorWindow).GetMethod("IsSelectedTab", BindingFlags.NonPublic | BindingFlags.Instance);

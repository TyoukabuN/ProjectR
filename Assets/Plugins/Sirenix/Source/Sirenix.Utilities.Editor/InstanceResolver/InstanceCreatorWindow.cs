//-----------------------------------------------------------------------
// <copyright file="InstanceCreatorWindow.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    internal class InstanceCreatorWindow : EditorWindow
    {
        ...
        private static bool CanCreateInstance(Type baseType, ref Type type)
        {
            ...
            if (baseTypeIsGenericType && (baseType.GetGenericTypeDefinition() == typeof(Dictionary<,>) || baseType.GetGenericTypeDefinition() == typeof(IDictionary<,>)))
            {
                if (type != typeof(Dictionary<,>) && type != typeof(NullType)) return false;
            }
        ...
        private TypeTreeNode GetTypeTree(Type type)
        {
            ...
            if (typeTrees.TryGetValue(type, out rootNode) == false)
            {
                ...
                rootNode = typeTrees[type] = new TypeTreeNode((Type)null)
                {
                    ChildNodes = AssemblyUtilities
                    .GetAllAssemblies()
                    .Select(x => new { assembly = x, flag = AssemblyUtilities.GetAssemblyTypeFlag(x) })
                    .GroupBy(a => a.flag)
                    .Select(a => new TypeTreeNode(a.Key)
                    {
                        IsVisible = (a.Key & AssemblyTypeFlags.UserTypes) != 0,
                        ChildNodes = AssemblyUtilities
                            .GetTypes(a.Key)
                            .Where(x => x != type && x != typeof(NullType))// ((a.Key & AssemblyTypeFlags.UserTypes) != 0 || x.IsPublic) && x != type) // Only include public types or types from User assembly.
                            ...
                            .GroupBy(t => ((t == typeof(NullType) || t.IsArray) ? type : t).GetAutoSelectionPath() ?? "ALSO!GROUP!ME")
                            .Select(g => new TypeTreeNode((g.Key == "ALSO!GROUP!ME") ? null : g.Key)
                            {
                                ChildNodes = g
                                    .Select(t => new TypeTreeNode(t) )
            ...
            #region Modified By Hunter (JB) -- 2022年2月28日

            if (type.GetAttribute<DoNotHideSelectionPathAttribute>() != null)
            {
                this.hideFoldoutLabels = false;
            }
        ...
        private class TypeTreeNode
        {
            ...
            public TypeTreeNode(AssemblyTypeFlags assemblyTypeFlags)
            {
                this.AssemblyTypeFlag = assemblyTypeFlags;
            }
            ...
            public void PrependType(Type type)
            {
                ...
                if (string.IsNullOrEmpty(specialType.GetAutoSelectionPath()))
                {

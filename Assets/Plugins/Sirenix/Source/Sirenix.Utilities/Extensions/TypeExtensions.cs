//-----------------------------------------------------------------------
// <copyright file="TypeExtensions.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public static class TypeExtensions
    {
        ...
        private static readonly Dictionary<Type, string> CachedBriefDisplayNames = new Dictionary<Type, string>();
        private static readonly Dictionary<Type, string> CachedPaths = new Dictionary<Type, string>();
        private static string GetCachedNiceName(Type type)
        {
            ...
            var displayName = GetCachedBriefDisplayName(type);
        ...
        public static string GetCachedBriefDisplayName(Type type)
        {
            if (!CachedBriefDisplayNames.TryGetValue(type, out var result))
            {
                var attr = type.GetAttribute<HunterClassLabelTextAttribute>();
                result = attr?.Text ?? null;
                CachedBriefDisplayNames[type] = result;
            }
        ...
        public static string GetSelectionPath(this Type type)
        {
            if (type == null) return string.Empty;
            if (!CachedPaths.TryGetValue(type, out var path))
            {
                path = type.GetAttribute<HunterSelectionPathAttribute>()?.path;
                ...
                CachedPaths[type] = path;
            }
            return path;
        }

        public static string GetAutoSelectionPath(this Type type)
        {
            var path = type.GetSelectionPath();
            if (string.IsNullOrWhiteSpace(path))
            {
                path = "~未分类";
            }
        ...
        public static string GetSelectionPathOrNameSpace(this Type type)
        {
            var path = type.GetSelectionPath();
        ...
        public static string GetFullPathAndName(this Type type)
        {
            return type.GetSelectionPath() + type.GetNiceName();
        }

        public static string GetDisplayPathAndDisplayName(this Type type)
        {
            var path = type.GetSelectionPath();
            ...
            return path + type.GetDisplayName();
        }
        #endregion

        public static string GetDisplayName(this Type type)
        {
            return GetCachedBriefDisplayName(type) ?? GetCachedNiceName(type);
        }
        ...
        {
            // NOTE: When modifying this method, also remember to modify Sirenix.Serialization.TypeExtensions.TryInferGenericParameters

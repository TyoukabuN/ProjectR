//-----------------------------------------------------------------------
// <copyright file="InspectorPropertyInfoUtility.cs" company="Sirenix ApS">
    ...
    public static class InspectorPropertyInfoUtility
    {
        ...
        public static List<InspectorPropertyInfo> CreateMemberProperties(InspectorProperty parentProperty, Type type, bool includeSpeciallySerializedMembers)
        {
            ...
            if (rootProperties.Count == 0)
            {
                ...
                // The above PopulateUnityProperties, uses the SerializeObject to find out which properties to show, but that doesn't take Odin's [ShowInInspector] attribtues into account.

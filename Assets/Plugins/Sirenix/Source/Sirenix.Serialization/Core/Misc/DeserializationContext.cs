//-----------------------------------------------------------------------
// <copyright file="DeserializationContext.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public sealed class DeserializationContext : ICacheNotificationReceiver
    {
        ...
        //模仿fs回调机制
        private Stack<IOdinSerializationCollector> internalCollectorStack = new Stack<IOdinSerializationCollector>(2);
        ...
        public void PushCollector(IOdinSerializationCollector collector)
        {
            this.internalCollectorStack.Push(collector);
            collector.OnPush();
        }

        public void PopCollector(IOdinSerializationCollector collector)
        {
            if (internalCollectorStack.Peek() != collector)
            {
                this.Config.DebugContext.LogError($"collector stacking error: {collector.GetType().Name}");
            }
            else
            {
                internalCollectorStack.Pop().OnPop();
            }
        }

        public void AddCollectable(IOdinSerializationCollectable collectable)
        {
            if (internalCollectorStack.TryPeek(out var c))
            {
                c.OnCollect(collectable);
            }
        }
        ...
        public void Reset()
        {
            ...
            this.internalCollectorStack.Clear();

//-----------------------------------------------------------------------
// <copyright file="OdinEditor.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public class OdinEditor : Editor
    {
        ...
        {
            // This collapses layout margins.
            // We do this because child layout GetRect calls can collaps parent marings.
            // Which means everything can start jumping around when you animate fade groups etc..
            // So instead, we collapse everything up front, and use Layout to push margins back, instead of using GUIStyle.maring (Which is collapsable)
            var margins = SirenixGUIStyles.PropertyMargin.margin;
            GUILayout.BeginHorizontal(SirenixGUIStyles.None); // Using none collapses margins.
        ...
        private static EditorWindow[] GetAllInspectorWindows()
        {
            ...
            if (inspectorWindowType == null) throw new Exception("UnityEditor.InspectorWindow type no longer exists in this version of Unity.");

            var objs = Resources.FindObjectsOfTypeAll(inspectorWindowType);

            var windows = new EditorWindow[objs.Length];

            for (int i = 0; i < objs.Length; i++)
            {
                windows[i] = (EditorWindow)objs[i];
            }
        ...
        protected virtual void OnEnable()
        {
            ...
            var inspectorWindows = GetAllInspectorWindows();

            for (int i = 0; i < inspectorWindows.Length; i++)
            {
                var window = inspectorWindows[i];

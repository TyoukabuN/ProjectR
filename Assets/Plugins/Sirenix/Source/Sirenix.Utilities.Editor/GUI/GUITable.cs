//-----------------------------------------------------------------------
// <copyright file="GUITable.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public partial class GUITable
    {
        ...
        #region Modified By Hunter (jb) -- 2022年10月26日 浮动表头
        public Vector2 DrawTableWithFloatingHeader(Vector2 scroll)
        {
             var e = Event.current.type;
            ...
            bool shouldDrawFloatingHeader = rowHeights.Length > 0 && scroll.y > rowHeights[0] * 0.3f;
            GUIHelper.PushIndentLevel(0);
            if (!shouldDrawFloatingHeader)
            {
                // Draw Cells:
                for (int x = 0; x < this.ColumnCount; x++)
                {
            ...
            {
                var maskRect = tableRect;
                var headerY = scroll.y;
                maskRect.height = rowHeights[0];
                maskRect = maskRect.AddY(headerY);
                
                if (e == EventType.MouseDown && maskRect.Contains(Event.current.mousePosition))
                {
                    Event.current.Use();
                    scroll.y = 0f;
                }
                void DrawCellsButHeader()
                {
                    for (int x = 0; x < this.ColumnCount; x++)
                    {
                        for (int y = 0; y < this.RowCount; y++)
                        {
                            if (y == 0 && shouldDrawFloatingHeader)
                            {
                                continue;
                            }
                            var cell = this.cells[x, y];
                ...
                titleBgColor.a = 0.7f;
                SirenixEditorGUI.DrawSolidRect(maskRect, titleBgColor);
                SirenixEditorGUI.DrawBorders(maskRect, 1, 1, 1, 1, SirenixGUIStyles.BorderColor);
                for (int x = 0; x < this.ColumnCount; x++)
                {
                    var cell = this.cells[x, 0];
                    if (cell != null)
                    {
                        var assignRect = cell.Rect;
                        assignRect = assignRect.AddY(headerY);
                        cell.DrawWithRect(assignRect);
                        SirenixEditorGUI.DrawBorders(assignRect, 1, 1, 1, 1, SirenixGUIStyles.BorderColor);
                    }
                }
            }

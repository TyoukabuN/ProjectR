//-----------------------------------------------------------------------
// <copyright file="PropertyValueEntry.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public abstract class PropertyValueEntry : IPropertyValueEntry
    {
        ...
        public bool ValueChangedFromPrefab
        {
            get 
            { 
                if (!this.valueChangedFromPrefab.HasValue)
                {
                    this.RefreshUnityPrefabModificationState();
                }

                return this.valueChangedFromPrefab.HasValue ? this.valueChangedFromPrefab.Value : false;
            }
        }
        ...
        public bool ListLengthChangedFromPrefab
        {
            get
            {
                if (!this.listLengthChangedFromPrefab.HasValue)
                {
                    this.RefreshUnityPrefabModificationState();
                }

                return this.listLengthChangedFromPrefab.HasValue ? this.listLengthChangedFromPrefab.Value : false;
            }
        }
        ...
        public void RefreshPrefabModificationState()
        {
            ...
            {
                var change = this.Property.Tree.PrefabModificationHandler.GetPrefabModificationType(this.Property);

                this.valueChangedFromPrefab = change == PrefabModificationType.Value;
                this.listLengthChangedFromPrefab = change == PrefabModificationType.ListLength;
                this.DictionaryChangedFromPrefab = change == PrefabModificationType.Dictionary;
            }
            else if (this.SerializationBackend.IsUnity)
            {
                this.valueChangedFromPrefab = null;
                this.listLengthChangedFromPrefab = null;
            }
        }
        ...
        private void RefreshUnityPrefabModificationState()
        {
            ...
            this.valueChangedFromPrefab = false;
            this.listLengthChangedFromPrefab = false;

            var prop = this.Property;

            if (prop.ChildResolver.IsCollection)
            {
                if (this.cachedUnityPropertyListLengthModificationPath == null)
                {
                    this.cachedUnityPropertyListLengthModificationPath = prop.UnityPropertyPath + ".Array.size";
                }

                for (int i = 0; i < count; i++)
                {
                    mod = prop.Tree.PrefabModificationHandler.GetUnityPropertyModification(this.cachedUnityPropertyListLengthModificationPath, i, out childrenHaveModifications);

                    if (mod != null)
                    {
                        this.listLengthChangedFromPrefab = true;
                        break;
                    }
                }
            }
            ...
            for (int i = 0; i < count; i++)
            {
                mod = prop.Tree.PrefabModificationHandler.GetUnityPropertyModification(prop.UnityPropertyPath, i, out childrenHaveModifications);

                if (mod != null || (this.IsMarkedAtomic && childrenHaveModifications))
                {
                    this.valueChangedFromPrefab = true;
                    break;
                }
            }
        }
        ...
        protected sealed override void UpdateValues()
        {
            for (int i = 0; i < this.ValueCount; i++)
            {
                      ...
                      && !object.ReferenceEquals(obj, null))
                {
                    ...
                    var newValue = EditorUtility.InstanceIDToObject(instanceId);

                    if (newValue != null && !object.ReferenceEquals(value, newValue) && newValue is TValue newTValue)
                    {
                        value = newTValue;
                    }
                }

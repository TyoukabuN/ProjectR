...
#pragma warning disable
    ...
    public sealed class ValueDropdownAttributeDrawer : OdinAttributeDrawer<ValueDropdownAttribute>
    {
            ...
            this.getValues = () =>
            {
                ...
                return value == null ? null : (value as IEnumerable)
                    ...
                    .Select(x =>
                    {
                        ...
                        #region Hunter
                        if (x.GetType().IsEnum || x.GetType() is string)
                        {
                            var labelText = x.GetType().GetField(x.ToString()).GetAttribute<LabelTextAttribute>();
                            if (labelText != null)
                                return new ValueDropdownItem(labelText.Text, x);
                        }
                        #endregion
        ...
        {
            bool isUniqueList = this.Attribute.IsUniqueList; // /*(this.Property.ChildResolver is IOrderedCollectionResolver) == false ||*/ (this.Attribute.IsUniqueList || this.Attribute.ExcludeExistingValuesInList);
            ...
            if (this.isList && !this.Attribute.ExcludeExistingValuesInList && isUniqueList)
            {
        ...
        private string GetCurrentValueName()
        {
            if (!EditorGUI.showMixedValue)
            {
                ...
                #region Modified By Hunter (jb) -- 2022年11月10日

                if (this.Attribute.DrawTypeNiceName)
                {
                    name = weakValue?.GetType()?.GetNiceName();
                }

//-----------------------------------------------------------------------
// <copyright file="CollectionDrawer.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public class CollectionDrawer<T> : OdinValueDrawer<T>, IDefinesGenericMenuItems, IHackyListDrawerInteractions
    {
        ...
        private class FilteredPropertyChildren
        {
            ...
            public void Update()
            {
                if (this.SearchFilter == null || SearchFilter.CheckSearchIsEmtpy())
                {
        ...
        protected override void Initialize()
        {
            ...
            #region Modified By Hunter (jwaybee) -- Sunday, January 9, 2022

            var htSearchAttr = this.Property.GetAttribute<HunterSearchableAttribute>();

            if (htSearchAttr != null)
            {
                searchFilter = new HunterPropertySearchFilter(Property.Parent, htSearchAttr);
                resolver.OnAfterChange += (change) =>
                {
                    this.Property.Children.Update();
                    this.info.FilteredChildren.Update();
                };
            }
            else if (searchAttr != null)
            {
            ...
            if (this.info.CustomListDrawerOptions.ElementColor != null)
            {
                this.info.ElementColor = ValueResolver.Get<Color>(this.Property, this.info.CustomListDrawerOptions.ElementColor, new ValueResolvers.NamedValue("index", typeof(int)), new ValueResolvers.NamedValue("defaultColor", typeof(Color)));
            }
        ...
        private void DrawToolbar()
        {
            SirenixEditorGUI.BeginHorizontalToolbar();
            {
                ...
                if (this.info.FilteredChildren.SearchFilter != null)
                {
                    ...
                    #region Modified By Hunter (jb) -- Sunday, January 9, 2022 支持列表自定义筛选
                    ...
                    var searchChanged = this.info.FilteredChildren.SearchFilter.DrawCollectionSearchField(this.info.SearchFieldControlName);
                    // var newTerm = SirenixEditorGUI.SearchField(rect, this.info.FilteredChildren.SearchFilter.SearchTerm, false, this.info.SearchFieldControlName);
                    
                    if (searchChanged)
                    {
                        if (!string.IsNullOrEmpty(info.FilteredChildren.SearchFilter.SearchTerm))
                        {
                            //this.info.Toggled.Value = true;
                            this.Property.State.Expanded = true;
                        }
            ...
            var rect = SirenixEditorGUI.BeginVerticalList();
            {
                for (int i = 0, j = from, k = from; j < to; i++, j++)
                {
                    var dragHandle = this.BeginDragHandle(j, i);
                    {
                        ...
                        {
                            k++;
                            this.DrawItem(this.info.FilteredChildren[j], dragHandle, evenColor, oddColor);
                        }
                        else
                        {
                            //GUILayout.Space(3);
        ...
        private Rect DrawItem(InspectorProperty itemProperty, DragHandle dragHandle, Color evenColor, Color oddColor)
        {
            ...
            if (this.info.ElementColor != null && !this.info.ElementColor.HasError)
            {
                this.info.ElementColor.Context.NamedValues.Set("index", index);
                this.info.ElementColor.Context.NamedValues.Set("defaultColor", color);
                color = this.info.ElementColor.GetValue();
            }

            Rect rect;
            rect = SirenixEditorGUI.BeginListItem(false, this.info.ListItemStyle, color, color, color, color, listItemOptions);
            {

...
#if UNITY_EDITOR
//-----------------------------------------------------------------------
// <copyright file="InspectorTypeDrawingConfigDrawer.cs" company="Sirenix ApS">
...
#pragma warning restore 0618
        ...
        private static readonly Dictionary<Type, Type> TypeToDrawingEditorMap = new Dictionary<Type,Type>(FastTypeComparer.Instance);
        ...
        static InspectorTypeDrawingConfigDrawer()
        {
            ...
            using (ProfileSection.Start("InspectorTypeDrawingConfigDrawer static constructor"))
            {
                ...
                {
                    var networkView = typeof(UnityEngine.Object).Assembly.GetType("UnityEngine.NetworkView"); // AssemblyUtilities.GetTypeByCachedFullName("UnityEngine.NetworkView");
                    if (networkView != null) NeverDrawTypes.Add(networkView);
                    var guiText = typeof(UnityEngine.Object).Assembly.GetType("UnityEngine.GUIText"); //AssemblyUtilities.GetTypeByCachedFullName("UnityEngine.GUIText");
                ...
                {
                    var types = TypeCache.GetTypesDerivedFrom(typeof(UnityEngine.Object));
                ...
                using (ProfileSection.Start("Assign editors to Unity objects"))
                {
                    ...
                    foreach (var type in unityObjectTypes)
                    {
                        if (!type.IsAbstract /*&& !type.IsGenericTypeDefinition && !type.IsGenericType*/ && !typeof(Editor).IsAssignableFrom(type) && !typeof(EditorWindow).IsAssignableFrom(type))
                        {
                            ...
                            TypeToDrawingEditorMap[type] = preExistingEditorType;
                        }
                    }
                }
        ...
        public static HashSet<Type> GetAllTypesDrawnByOdin()
        {
            ...
            foreach (var entry in TypeToDrawingEditorMap)
            {
                var editor = entry.Value;
                if (object.ReferenceEquals(editor, null)) continue;
                if (object.ReferenceEquals(editor, typeof(OdinEditor)) || typeof(OdinEditor).IsAssignableFrom(editor))
                {
        ...
        {
            var editor = GetActualDrawingEditorForType(drawnType);
            if (object.ReferenceEquals(editor, null)) return false;
            return object.ReferenceEquals(editor, typeof(OdinEditor)) || typeof(OdinEditor).IsAssignableFrom(editor);
        }

        public static Type GetActualDrawingEditorForType(Type drawnType)
        {
            Type editorType;
            TypeToDrawingEditorMap.TryGetValue(drawnType, out editorType);
            return editorType;
        }

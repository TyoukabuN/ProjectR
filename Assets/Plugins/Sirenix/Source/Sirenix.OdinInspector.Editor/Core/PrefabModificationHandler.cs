//-----------------------------------------------------------------------
// <copyright file="PrefabModificationHandler.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public sealed class PrefabModificationHandler : IDisposable
    {
        ...
        #region Modified By Hunter (jb) -- 2022年9月20日
        // 在没有双击打开PrefabVariance的资产的时候这个类里很多逻辑是用不了的, Unity 的PrefabUtility.GetPrefabType返回的枚举里没有PrefabVariance这个类型
        // 传PrefabVariance进去拿到的只是PrefabType.Prefab, 而Odin只会对PrefabInstance类型做Prefab相关的处理.
        // (Unity双击打开编辑PrefabVariance的时候应该是实例化了一个Prefab出来)
        // 总之这里要让Odin把PrefabVariance资产也处理了, 这个bool是开关
        public bool EnableVariancePrefabEditing { get; set; }
        #endregion
        ...
        public PrefabModificationHandler(PropertyTree tree)
        {
            ...
            this.unityModLookups = new PathLookup<PropertyModification>[tree.WeakTargets.Count];
            ...
            if (typeof(UnityEngine.Object).IsAssignableFrom(this.Tree.TargetType))
            {
                ...
                for (int i = 0; i < this.Tree.WeakTargets.Count; i++)
                {
                    ...
                    if (target != null)
                    {
                        ...
                        if ((prefabType == PrefabType.PrefabInstance && (prefab = OdinPrefabSerializationEditorUtility.GetCorrespondingObjectFromSource(target)) != null)
                            ||
                            (EnableVariancePrefabEditing && prefabType == PrefabType.Prefab && PrefabUtility.IsPartOfVariantPrefab(target) && (prefab = PrefabUtility.GetCorrespondingObjectFromSource(target)) !=null) 
                            ) 
                ...
                if (prefabCount > 0)
                {
                    ...
                    if (this.prefabPropertyTree != null)
                    {
                        ...
                        {
                            this.prefabPropertyTree.Dispose();
                        ...
                        else
                        {
                            for (int i = 0; i < this.prefabPropertyTree.WeakTargets.Count; i++)
                            {
                                ...
                                {
                                    this.prefabPropertyTree.SetTargets(prefabsNoNull);
                                    break;
                                }
                            }
                        }
        ...
        private void RebuildUnityPropertyModificationsLookup()
        {
            if (this.HasPrefabs && !Application.isPlaying)
            {
                for (int i = 0; i < this.Tree.WeakTargets.Count; i++)
                {
                    ...
                    if (targetPrefab != null)
                    {
                        ...
                        {
                            var lookup = this.unityModLookups[i];

                            if (lookup == null)
                            {
                                lookup = new PathLookup<PropertyModification>();
                                this.unityModLookups[i] = lookup;
                            }
            ...
            this.needsToRebuildPropertyModificationLookup = false;
        }
        ...
        public void CleanForCachedReuse()
        {
            if (this.prefabPropertyTree != null)
            {
                this.prefabPropertyTree.CleanForCachedReuse();
            }
        }
        ...
        {
            if (this.needsToRebuildPropertyModificationLookup)
            {
                this.RebuildUnityPropertyModificationsLookup();
            }

            childrenHaveModifications = false;

            var lookup = this.unityModLookups[selectionIndex];
            if (lookup == null) return null;

            PropertyModification mod;
            StringSlice nearestPath;

            if (lookup.TryGetValue(path, out nearestPath, out childrenHaveModifications, out mod))
            {
                return mod;
            }

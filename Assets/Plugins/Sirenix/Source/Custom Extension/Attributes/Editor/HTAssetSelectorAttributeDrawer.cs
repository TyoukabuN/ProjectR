...
#pragma warning disable
    ...
    public sealed class HTAssetSelectorAttributeDrawer : OdinAttributeDrawer<HTAssetSelectorAttribute>
    {
        private ValueResolver<string> getterPathValue;
        private ValueResolver<string> getterFilterValue;
        ...
        protected override void Initialize()
        {
            ...
            if (this.Attribute.PathGetter != null)
            {
                this.getterPathValue = ValueResolver.Get<string>(this.Property, this.Attribute.PathGetter);
                var path = this.getterPathValue.GetValue();
                // Debug.Log($"path : {path}");
            ...
            {
                paths = this.Attribute.SearchInFolders ?? new string[0];
            }

            string filter = "";
            if (this.Attribute.FilterGetter != null)
            {
                this.getterFilterValue = ValueResolver.Get<string>(this.Property, this.Attribute.FilterGetter);
                var path = this.getterFilterValue.GetValue();
            }
            else
            {
                filter = this.Attribute.Filter ?? "";
            }
            ...
            this.getValues = () =>
            {
                ...
                var assetGuids = AssetDatabase.FindAssets(filter, paths);

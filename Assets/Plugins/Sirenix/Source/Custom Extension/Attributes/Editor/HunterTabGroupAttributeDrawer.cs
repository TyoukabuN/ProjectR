...
#pragma warning disable
    ...
    /// <summary>
    /// Draws all properties grouped together with the <see cref="HunterTabGroupAttribute"/>
    /// </summary>
    /// <seealso cref="HunterTabGroupAttribute"/>
    public class HunterTabGroupAttributeDrawer : OdinGroupDrawer<HunterTabGroupAttribute>, IOnSelfStateChangedNotification
    {
        public const string CurrentTabIndexKey = "CurrentTabIndex";
        public const string CurrentTabNameKey = "CurrentTabName";
        
        private static readonly Sirenix.OdinInspector.Editor.ActionResolvers.NamedValue[] ActionArgs = new Sirenix.OdinInspector.Editor.ActionResolvers.NamedValue[]
        {
            new Sirenix.OdinInspector.Editor.ActionResolvers.NamedValue("tabName", typeof(string))
        };
        ...
        private ActionResolver onTabChangeAction;
        ...
        protected override void Initialize()
        {
            ...
            for (int j = 0; j < this.Property.Children.Count; j++)
            {
                ...
                if (child.Info.PropertyType == PropertyType.Group)
                {
                    ...
                    if (attrType.IsNested && attrType.DeclaringType == typeof(HunterTabGroupAttribute))
                    {
            ...
            this.onTabChangeAction = ActionResolver.Get(this.Property, this.Attribute.OnTabChangeAction, ActionArgs);
        ...
        public void OnSelfStateChanged(string state)
        {
            ...
            if (state == CurrentTabIndexKey)
            {
                ...
                this.onTabChangeAction.Context.NamedValues.Set("tabName",this.tabs[index].TabName);
                this.onTabChangeAction.DoAction();
            }

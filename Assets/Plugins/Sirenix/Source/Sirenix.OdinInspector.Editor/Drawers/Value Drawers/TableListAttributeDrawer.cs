//-----------------------------------------------------------------------
// <copyright file="TableListAttributeDrawer.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public class TableListAttributeDrawer : OdinAttributeDrawer<TableListAttribute>
    {
        ...
        private void AddColumns(int rowIndexFrom, int rowIndexTo)
        {
            ...
            {
                int skip = 0;
                InspectorProperty rowProperty = this.Property.Children[y];
                for (int x = 0; x < rowProperty.Children.Count; x++)
                {
                    InspectorProperty colProperty = rowProperty.Children[x];
                    if (this.seenColumnNames.Add(colProperty.Name))
                    {
                        var hide = GetColumnAttribute<HideInTablesAttribute>(colProperty);
                        if (hide != null)
                        {
                            skip++;
                            continue;
                        ...
                        var colAttr = GetColumnAttribute<TableColumnWidthAttribute>(colProperty);
                        if (colAttr != null)
                        {

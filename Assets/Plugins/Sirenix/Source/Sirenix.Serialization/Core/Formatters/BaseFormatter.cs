//-----------------------------------------------------------------------
// <copyright file="BaseFormatter.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public abstract class BaseFormatter<T> : IFormatter<T>
    {
        ...
        //模仿fs的机制
        protected static readonly bool ImplementsISerializationCollector = typeof(T).ImplementsOrInherits(typeof(IOdinSerializationCollector));
        protected static readonly bool ImplementsISerializationCollectable = typeof(T).ImplementsOrInherits(typeof(IOdinSerializationCollectable));
        
        //表明一个类可以在反序列化中调初始化constructor
        protected static readonly bool AllowInitializeOnDeserialization = typeof(T).IsDefined<OdinInitOnDeserialization>(true);
        ...
        public T Deserialize(IDataReader reader)
        {
            ...
            T value;
            if (AllowInitializeOnDeserialization)
            {
                try
                {
            ...
            if (ImplementsISerializationCollector)
            {
                context.PushCollector(value as IOdinSerializationCollector);
            }
            ...
            #region Modified By Hunter (jb) -- 2022年8月22日
            //模仿fs处理序列化子类回调
            if (ImplementsISerializationCollectable)
            {
                context.AddCollectable(value as IOdinSerializationCollectable);
            }
            
            if (ImplementsISerializationCollector)
            {
                context.PopCollector(value as IOdinSerializationCollector);
            }
            #endregion

//-----------------------------------------------------------------------
// <copyright file="PropertyContextMenuDrawer.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    using SerializationUtility = Sirenix.Serialization.OdinSerializationUtility;
    ...
    public static class PropertyContextMenuDrawer
    {
        ...
        private static void PopulateGenericMenu(InspectorProperty property, GenericMenu genericMenu)
        {
            ...
            if (property.Parent != null && property.Parent.ChildResolver is IOrderedCollectionResolver)
            {
                ...
                {
                    genericMenu.AddDisabledItem(new GUIContent("移动元素到顶部"));//Move element to top
                    genericMenu.AddDisabledItem(new GUIContent("移动元素到底部"));//Move element to bottom
                    genericMenu.AddDisabledItem(new GUIContent("移动元素到指定下标"));//Move element to index
                    genericMenu.AddDisabledItem(new GUIContent("复制元素"));//Duplicate element
                    genericMenu.AddDisabledItem(new GUIContent("插入已复制的元素"));//Insert pasted element
                    genericMenu.AddDisabledItem(new GUIContent("插入新元素"));//Insert new element
                    genericMenu.AddDisabledItem(new GUIContent("删除元素"));//Delete element
                }
                else
                {
                    //Move element to top
                    genericMenu.AddItem(new GUIContent("移动元素到顶部"), false, () =>
                    {
                    ...
                    genericMenu.AddItem(new GUIContent("移动元素到底部"), false, () =>
                    {
                        var values = new object[property.ValueEntry.WeakValues.Count];
                    ...
                    genericMenu.AddItem(new GUIContent("移动元素到指定下标"), false, () =>
                    {
                    ...
                    //两种复制，一个复制是完全复制，一个复制是带有回调的复制
                    //Duplicate element
                    genericMenu.AddItem(new GUIContent("复制元素并添加至下方(完全复制)"), false, () =>
                    {
                        var values = new object[property.ValueEntry.WeakValues.Count];
                    ...
                    genericMenu.AddItem(new GUIContent("复制元素并添加至下方(带有回调的复制,比如包含了GUID的更新)"), false, () =>
                    {
                        var values = new object[property.ValueEntry.WeakValues.Count];
                        for (int i = 0; i < values.Length; i++)
                        {
                            values[i] = SerializationUtility.CreateCopy(property.ValueEntry.WeakValues[i]);
                            if(values[i] is IOdinListCopyElementHandle handle)
                                handle.CopyCallBack(ref values[i]);
                        }
                    ...
                    {
                        //Insert pasted element
                        genericMenu.AddItem(new GUIContent("插入已复制的元素"), false, () =>
                        {
                    ...
                    {
                        //Insert new element
                        genericMenu.AddItem(new GUIContent("插入新元素"), false, () =>
                        {
                            property.Tree.DelayActionUntilRepaint(() =>
                            {
                    ...
                    //Delete element
                    genericMenu.AddItem(new GUIContent("删除元素"), false, () =>
                    {
                        property.Tree.DelayActionUntilRepaint(() =>
                        {
                            parentResolver.QueueRemoveAt(property.Index);
            ...
            {
                genericMenu.AddDisabledItem(new GUIContent("粘贴"));//Paste
            }
            ...
            if (hasValue)
            {
                ...
                {
                    //Copy
                    genericMenu.AddItem(new GUIContent("复制"), false, () => Clipboard.Copy(valueToCopy, CopyModes.CopyReference));
                }
                else if (!property.ValueEntry.SerializationBackend.SupportsCyclicReferences)
                {
                    //Copy
                    genericMenu.AddItem(new GUIContent("复制"), false, () => Clipboard.Copy(valueToCopy, CopyModes.DeepCopy));
                }
                else
                {
                    genericMenu.AddItem(new GUIContent("复制"), false, () => Clipboard.Copy(valueToCopy, CopyModes.DeepCopy));//Copy
                    genericMenu.AddItem(new GUIContent("特殊复制/Deep Copy (default)"), false, () => Clipboard.Copy(valueToCopy, CopyModes.DeepCopy));//Copy Special
                    genericMenu.AddItem(new GUIContent("特殊复制/Shallow Copy"), false, () => Clipboard.Copy(valueToCopy, CopyModes.ShallowCopy));
                    genericMenu.AddItem(new GUIContent("特殊复制/Copy Reference"), false, () => Clipboard.Copy(valueToCopy, CopyModes.CopyReference));
                }
            }
            else
            {
                genericMenu.AddDisabledItem(new GUIContent("复制"));//Copy
            }
            ...
            if (isNullable)
            {
                ...
                {
                    //Set To Null
                    genericMenu.AddItem(new GUIContent("设置此元素值为空"), false, () =>
                    {
                        property.Tree.DelayActionUntilRepaint(() =>
                        {
                            for (int i = 0; i < property.ValueEntry.ValueCount; i++)
                ...
                {
                    genericMenu.AddDisabledItem(new GUIContent("设置此元素值为空"));//Set To Null
                }
            }
            ...
            if (hasValue)
            {
                ...
                genericMenu.AddItem(new GUIContent($"编辑脚本{type}"), false, () =>
                {
                    //var type = property.ValueEntry.TypeOfValue;
                    EditScriptUtitlity.OpenScriptOfType(type);
                });
            }
            else
            {
                genericMenu.AddDisabledItem(new GUIContent("编辑脚本"));//Edit Script
            }
        }
    }

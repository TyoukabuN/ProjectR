//-----------------------------------------------------------------------
// <copyright file="InspectorProperty.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public sealed class InspectorProperty : IDisposable
    {
        ...
        public void RecordForUndo(string message = null, bool forceCompleteObjectUndo = false)
        {
            ...
            if (!this.Tree.RecordUndoForChanges) return;
        ...
        {
            if (this.ValueEntry == null || this.BaseValueEntry == null) return null;
            if (this.ValueEntry is IPropertyValueEntry<T> result1) return result1;
            if (this.BaseValueEntry is IPropertyValueEntry<T> result2) return result2;
            if (!typeof(T).IsAssignableFrom(this.ValueEntry.TypeOfValue))
            {
                var t1 = typeof(T);
                var t2 = this.ValueEntry.TypeOfValue;

                // Debug.Log(t1.GetNiceFullName() + " : " + t2.GetNiceFullName());

                return null;
            }
            return (IPropertyValueEntry<T>)PropertyValueEntry.CreateAlias(this.BaseValueEntry, typeof(T));
        }
            ...
            if (this.drawerChains.Count <= index)
            {
                ...
                for (int i = 0; i < result.BakedDrawerArray.Length; i++)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            // We must rethrow this later, after we are done initializing drawers
                            // Otherwise our state will be corrupted
        ...
        public InspectorProperty GetFirstParentPropertyOfType(Type type, int budget)
        {
            var current = this.Parent;
            int c = 0;
            while (current != null && c < budget)
            {
                if (current.ValueEntry != null && type.IsAssignableFrom(current.ValueEntry.TypeOfValue))
                {
                    return current;
                }
                c++;
        ...
        public TValue GetFirstParentOfValue<TValue>(int budget)
        {
            var current = this.Parent;
            int c = 0;
            while (current != null && c < budget)
            {
                if (current.ValueEntry != null && current.ValueEntry.WeakSmartValue is TValue t) return t;
                c++;
                current = current.Parent;
            }
            return default;
        ...
        #region Modified By Hunter (jb) -- 2023年6月30日

        public InspectorProperty FindChildWithReference(object referenceObject)
        {
            return FindChild((x => x.ValueEntry != null && ReferenceEquals(x.ValueEntry.WeakSmartValue, referenceObject)), true);
        }
        ...
        public bool Update(bool forceUpdate = false)
        {
            ...
            if (this.stateUpdaters == null || this.Children == null || this.ChildResolver == null)
            {
                this.RefreshSetup(false);
            }
            else if (this.ValueEntry != null && this.ChildResolver.ResolverForType != null && this.ValueEntry.TypeOfValue != this.ChildResolver.ResolverForType)
            {
                this.RefreshSetup(true);
            }
            else
            ...
            {
                this.BaseValueEntry.RefreshPrefabModificationState();
            }
        ...
        public void PopulateGenericMenu(GenericMenu genericMenu)
        {
            ...
            if (this.Tree.TreeIsSetupForIMGUIDrawing_TEMP_INTERNAL)
            {
                ...
                var validatorComponent = GetComponent<ValidationComponent>();

                if (validatorComponent != null)
                {
                    var validatorList = validatorComponent.GetValidators();
                    var length = validatorList.Count;

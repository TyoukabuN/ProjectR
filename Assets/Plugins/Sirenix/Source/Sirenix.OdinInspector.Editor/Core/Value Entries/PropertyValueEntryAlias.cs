//-----------------------------------------------------------------------
// <copyright file="PropertyValueEntryAlias.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public sealed class PropertyValueEntryAlias<TActualValue, TValue> : PropertyValueEntryAlias, IPropertyValueEntry<TValue>, IValueEntryActualValueSetter<TValue> //where TValue : TActualValue
    {
        ...
        public TValue SmartValue
        {
            get
            {
                var value = this.entry.SmartValue;
                ...
                throw new InvalidCastException($"Cannot cast from actual type '{(this.entry.SmartValue?.GetType()?.GetNiceFullName() ?? "null value")}' to expected alias type '{typeof(TValue).GetNiceFullName()}'.");
            }
            set
            {
                if (ValueIsMarkedAtomic)
                ...
                else if (ValueIsPrimitive || ValueIsValueType)
                {
                    ...
                    var existingValue = this.entry.Values[0];

                    if (object.ReferenceEquals(null, existingValue) || existingValue.GetType() != typeof(TValue) || !EqualityComparer(value, (TValue)(object)existingValue))
                    {
                        this.entry.SmartValue = (TActualValue)(object)value;
                    }
                }
                else
                {
        ...
        public override void Update()
        {
            ...
            if (ValueIsMarkedAtomic)
            {
                for (int i = 0; i < this.ValueCount; i++)
                {
                    try
                    {
                        if (!(this.entry.Values[i] is TValue value)) throw new InvalidCastException();
        ...
        public bool ValueIsPrefabDifferent(TValue value, int index)
        {
            ...
            if (!(value is TActualValue val)) throw new InvalidCastException();
            return this.entry.ValueIsPrefabDifferent(val, index);
        }

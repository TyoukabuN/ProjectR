...
#pragma warning disable
    ...
    public class OdinMenuTree : IEnumerable
    {
            ...
            if (Event.current.delta != new Vector2())
            {
                ...
                // TODO: calculate start and end index based on GUIClipInfo.VisibleRect.
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        GUI.enabled = !item.DisbableInMenu;
                ...
                else
                {
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        GUI.enabled = !tree[i].DisbableInMenu;
        ...
        private string DrawSearchField(Rect rect, string searchTerm, bool autoFocus)
        {
            ...
            searchTerm = this.searchField.OnGUI(rect, searchTerm);
...
#if UNITY_EDITOR
        public int FindNearestItemAtPos(Vector2 mousePos)
        {
            var rootItemsCount = RootMenuItem.ChildMenuItems.Count;
            
            for (int i = 0; i < rootItemsCount; i++)
            {
                var item = RootMenuItem.ChildMenuItems[i];
                if (i == 0)
                {
                    if (mousePos.y < item.rect.yMin) return 0;
                }

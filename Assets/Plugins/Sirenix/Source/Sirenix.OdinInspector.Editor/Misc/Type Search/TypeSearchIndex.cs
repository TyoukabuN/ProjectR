//-----------------------------------------------------------------------
// <copyright file="TypeSearchIndex.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public sealed class TypeSearchIndex
    {
        ...
        public static readonly TargetMatchCategory[] AttributeValueMatchCategoryArray = new TargetMatchCategory[2] { TargetMatchCategory.Attribute, TargetMatchCategory.Value };
        public static readonly TargetMatchCategory[] ValueAttributeMatchCategoryArray = new TargetMatchCategory[2] { TargetMatchCategory.Value, TargetMatchCategory.Attribute };
        ...
        public class TypeMatchCacheSignatureEqualityComparer : IEqualityComparer<TypeMatchQuery>
        {
            ...
            {
                if (!typeArrayEqualityComparer.Equals(x.Targets, y.Targets)) return false;

                if (object.ReferenceEquals(x.Categories, y.Categories)) return true;

                if (x.Categories != null)
                {
                    if (x.Categories.Length != y.Categories.Length) return false;

                    for (int i = 0; i < x.Categories.Length; i++)
                    {
                        if (x.Categories[i] != y.Categories[i]) return false;
                    }
                }
            ...
            {
                var hash = typeArrayEqualityComparer.GetHashCode(obj.Targets);
        ...
        {
            var result = new List<TypeMatchQuery>();
        ...
        public TypeSearchResult[] GetMatches(Type target, TargetMatchCategory category)
        {
            ...
            lock (this.LOCK)
            {
                var query = CachedQuery1;

                query.Targets[0] = target;
                query.Categories[0] = category;

                if (!this.resultCache.TryGetValue(query, out result))
                {
                    query = new TypeMatchQuery
                    { 
                        Targets = new Type[] { target },
                        Categories = new TargetMatchCategory[] { category },
                    };

                    result = this.FindAllMatches(query.Targets, query.Categories);
                    this.resultCache[query] = result;
                }
            }
        ...
        public TypeSearchResult[] GetMatches(Type target1, Type target2, TargetMatchCategory category1, TargetMatchCategory category2)
        {
            ...
            lock (this.LOCK)
            {
                ...
                if (!this.resultCache.TryGetValue(query, out result))
                {
                    query = new TypeMatchQuery
                    {
                        Targets = new Type[] { target1, target2 },
                        Categories = new TargetMatchCategory[] { category1, category2 },
                    };
        ...
        public TypeSearchResult[] GetMatches(Type[] targets, TargetMatchCategory[] categories)
        {
            ...
            lock (this.LOCK)
            {
                ...
                if (!this.resultCache.TryGetValue(query, out result))
                {
                    result = this.FindAllMatches(targets, categories);
                    this.resultCache[query] = result;
                }
            }
        ...
        private TypeSearchResult[] FindAllMatches(Type[] targets, TargetMatchCategory[] categories)
        {
            ...
            for (int i = 0; i < this.indexedTypes.Count; i++)
            {
                ...
                var cats = info.Info.TargetCategories;

                // Remove matching cases based on categories
        ...
        public void AddDefaultMatchCreators()
        {
            lock (this.LOCK)
            {
                ...
                this.TypeMatcherCreators.Add(new DerivedTypeMatcher.Creator());

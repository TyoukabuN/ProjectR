//-----------------------------------------------------------------------
// <copyright file="OdinMenuTreeExtensions.cs" company="Sirenix ApS">
...
#pragma warning disable
    ...
    public static class OdinMenuTreeExtensions
    {
        ...
        public static IEnumerable<OdinMenuItem> AddAllAssetsAtPathCombined(this OdinMenuTree tree, string menuPath, string assetFolderPath, Type type, bool includeSubDirectories = false)
        {
            ...
            var assetFolderPathLower = assetFolderPath.ToLower();
        ...
        #region Hunter
        public delegate string OnAddAllAssetsAtPathDlg(UnityEngine.Object obj, string name);
        #endregion
        ...
        /// <returns>Returns all menu items created in order to add the menu item at the specified menu item path.</returns>
        public static IEnumerable<OdinMenuItem> AddAllAssetsAtPath(this OdinMenuTree tree, string menuPath, string assetFolderPath, Type type, bool includeSubDirectories = false, bool flattenSubDirectories = false, OnAddAllAssetsAtPathDlg onAddAllAssetsEvent = null)
        {
            assetFolderPath = (assetFolderPath ?? "").TrimEnd('/') + "/";
            var assetFolderPathLower = assetFolderPath.ToLower();
            if (!assetFolderPathLower.StartsWith("assets/") && !assetFolderPathLower.StartsWith("packages/"))
            ...
            #region 插件修改--by cpx-- 2023/10/12

            //原因：AssetDatabase.GetAllAssetPaths()这个搜索算法底层是DFS，深度优先，出现的问题就是，找到的资源和引擎中资源的顺序不一致
            assets.Sort();
            ...
            foreach (var assetPath in assets)
            {
                ...
                #region 自定义文字
                string showName = onAddAllAssetsEvent?.Invoke(obj, name);

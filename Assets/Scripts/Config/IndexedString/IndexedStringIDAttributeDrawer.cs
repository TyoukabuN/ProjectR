...
{
    public class IndexedStringIDAttributeDrawer : OdinAttributeDrawer<IndexedStringIDAttribute, int>
    {
        ...
        protected override void DrawPropertyLayout(GUIContent label)
        {
            ...
            var itemAsset = IndexedString.Instance.GetConfig(id);
            EditorGUILayout.BeginHorizontal();
            if (itemAsset != null)
            {
                if (GUILayout.Button(itemAsset.Editor_LabelName))
                {
                    if (IndexedString.Instance.Editor_GetItemAsset(id) is { } a)
                    {
            ...
            {
                IndexedString.Instance.Editor_OpenItemCreateWindow((config) =>
                {
            ...
            if (GUILayout.Button("复制", GUILayout.Width(48)))
            {
                if (itemAsset != null)
                {
                    var errCode = IndexedString.Instance.Editor_CopyItemAsset(itemAsset, out var copy);
            ...
            {
                IndexedString.Selector.Show((config) =>
                {
                    ValueEntry.SmartValue = config.ID;
                    ValueEntry.ApplyChanges();
                });
            ...
            if (itemAsset != null)
            {
                ...
                {
                    var propertyTree = IndexedString.Instance.Editor_GetPropertyTreeByID(itemAsset.ID);
                    if (propertyTree != null)
                    {
                        ...
                        if (propertyTree.ApplyChanges())
                        {
                            ...
                            {
                                IndexedString.Instance.Editor_MaskItemAssetDirty(itemAsset.ID, false);
                            });
                        }
        ...
        public void ShowMoreMenu(IndexedStringItemAsset item)
        {
            IndexedString.Instance.Editor_ShowMoreMenu(menu =>
            {
                if (item != null)
                    menu.AddItem(new GUIContent("删除"), false,
                        () => { IndexedString.Instance.Editor_RemoveItem(item.ID); });
            });
        }
    }
}
